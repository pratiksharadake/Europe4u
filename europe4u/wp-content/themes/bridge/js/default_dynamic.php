...
foreach ($posts as $post) {
    if(get_post_meta($post->ID, "qode_show-animation", true) == "no_animation") :
        ?>
        no_ajax_pages.push('<?php echo get_permalink($post->ID) ?>');
    <?php
    endif;
}
...
if (isset($qode_options_proya['internal_no_ajax_links'])) {
	foreach (explode(',', $qode_options_proya['internal_no_ajax_links']) as $no_ajax_link) {
?>
...
?>
no_ajax_pages.push("<?php echo htmlspecialchars_decode(wp_logout_url()); ?>");
